...
export function usePostReactions(post: Post) {
  ...
  async function handleIncrementLike() {
    ...
    await fetch(`/api/reactions/${post.slug}`, {
      method: 'POST',
      body: JSON.stringify({
        reaction: 'like_count',
        type: 'increment'
  ...
  function getReactionsFromLocalStorage() {
    if (typeof window !== 'undefined') {
      return JSON.parse(localStorage.getItem(post.slug)) || initialReactionState;
    }
    return null;
  }
  ...
  function setReactionsToLocalStorage(reactions) {
    if (typeof window !== 'undefined') {
      localStorage.setItem(post.slug, JSON.stringify(reactions));
    }
  }
