...
import { HiMailOpen, HiSparkles } from 'react-icons/hi'
...
const navigation: NavigationItem[] = [
  { href: '/meagan', icon: MdWavingHand, name: 'About' },
...
function Navbar() {
  ...
  return (
    <>
      <header className='z-[100] flex items-center justify-between bg-transparent p-4 pb-0' variants={menuVariants}>
        <div>
          <Logo />
        </div>
        <nav className='hidden space-x-4 sm:flex sm:space-x-2'>
          {navigation.map(({ href, icon: Icon, name }) => (
            <Link
              ...
              href={href}
            >
              <Icon className='mr-2 hidden align-middle lg:inline-block' />
              <span>{name}</span>
            </Link>
          ))}
          <Link
            ...
            href='#'
          >
            <HiMailOpen className='mr-2 hidden align-middle lg:inline-block' />
