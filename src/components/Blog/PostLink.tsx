...
export const PostLink = ({ post }: PostLinkProps) => {
  ...
  const isHighlighted = useMemo(() => {
    return post.tags?.length && post.tags.length > 0 ? post.tags.map((t) => t.slug).indexOf('highlights') !== -1 : false
  }, [post.tags])

  return (
    <div className={clsx('my-2 flex justify-between p-1', isHighlighted ? 'bg-yellow-100 rounded' : '')}>
      <div className='flex flex-col'>
        <div className='flex flex-row items-center space-x-4'>
          {post.external ? (
            <a href={post.href} className='link--blue flex-grow text-lg font-semibold' target='_blank' rel='noreferrer'>
            ...
            <Link href={`/blog/${post.slug}`} className='link--blue flex-grow text-lg font-semibold'>
              <span>{post.title}</span>
